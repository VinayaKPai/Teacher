...
function divBody( $result, $nested, $status ) {
  // print_r($nested);

        foreach ( $result as $x ) {//$x is entire row of outer query ie each deployment details

          echo '<div class="panel panel-default">';
          $dId = $status."collapse".$x['Id'];
          if ($status!='all') {
            $displaypanel = '<div class="panel-heading">
          ...
          echo '<div id="'.$dId.'" class="pannel-collapse collapse">';
            ...
            echo '<div class="panel-body" style="background:  var(--BodyGradient);">';
                // if ($x['question']) {//process the question column data which is an array of numbers
